...
namespace S1ObjectDefinitions.LZ
{
	class Spikeball : ObjectDefinition
	{
		...
		{
			byte[] artfile = ObjectHelper.OpenArtFile("../../Objects/57 SYZ, LZ Spiked Ball and Chain/art/LZ Spiked Ball & Chain.nem", CompressionType.Nemesis);
			img = ObjectHelper.MapASMToBmp(artfile, "../../Objects/57 SYZ, LZ Spiked Ball and Chain/maps/Spiked Ball and Chain (LZ).asm", 1, 0);
			for (int i = 0; i < labels.Length; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(artfile, "../../Objects/57 SYZ, LZ Spiked Ball and Chain/maps/Spiked Ball and Chain (LZ).asm", labels[i], 0));
		}
